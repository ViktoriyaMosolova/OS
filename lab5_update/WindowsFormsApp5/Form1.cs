...
namespace WindowsFormsApp6
{
    public partial class Form1 : Form
    {
        ...
        void Alpaca()
        {
            var dll = Assembly.LoadFile(@"C:\Users\1\Desktop\OS\lab5\ClassLibrary2\bin\Debug\ClassLibrary2.dll");
            var the = dll.GetType("ClassLibrary2.FileExplorer");
            var c = Activator.CreateInstance(the);
            MethodInfo[] mi = the.GetMethods();
            mi[1].Invoke(c, new object[] { treeView1 });
        }
        void Capibara(object data)
        {
            ...
            { 
                foreach (string item in LogicalDrives.OutputGetLogicalDrives()) 
                {
                    textBox1.Text += item; 
                } 
            };
